...
# Import modules using absolute imports based on project_root
sys.path.insert(0, project_root)  # Ensure project root is first in path
from scripts.grid_optimization.functions import *
from scripts.grid_optimization.config_grid import *
from scripts.grid_optimization.cables import *
from scripts.grid_optimization.data_loading import load_data
from scripts.grid_optimization.data_extraction import extract_charger_counts
...
def run_optimization():
    """
    Main function to run the grid optimization process.
    This function is called from the main control script.
    """
    #------------------------------------------------------------------------------
    # SECTION 2: DATA LOADING AND PREPROCESSING
    #------------------------------------------------------------------------------
    # Use location from config instead of hard-coded coordinates
    config_location = Config.Traffic.Settings.DEFAULT_LOCATION
    ref_point = Point(
        config_location['LONGITUDE'],
        config_location['LATITUDE']
    )
    print(f"Using location from config: ({config_location['LONGITUDE']}, {config_location['LATITUDE']})")
    ...
    # Early definition to ensure it's available
